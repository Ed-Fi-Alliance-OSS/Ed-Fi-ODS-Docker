...
# Mounted into each tenant's db-ods-tenantN container; each tenant supplies its own
...
# files (see compose-multi-tenant-env.override.yml.example), the file-existence checks below
# skip template creation, so environments that don't use Instance Management are unaffected.
