...
# Mounted into each db-ods-YYYY container; each one supplies its own
...
# compose file (see the comment in compose-single-tenant-odscontext-env.yml), since there is no
# single ODS host for it to target. This script only pre-builds the template databases on each
# per-school-year ODS host, ahead of a future per-context wiring.
#
# This is optional: unless a SQL_BACKUPS_FOLDER volume is bind-mounted with the two SQL dump
# files (see compose-single-tenant-odscontext-env.override.yml.example), the file-existence
